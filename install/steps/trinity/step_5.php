...
$account = $Realm->query("SELECT * FROM `account` WHERE `username` LIKE ?", array($_POST['account']))->fetch_row();
if($account != FALSE) 
{
    $array = array(
        'id' => $account['id'],
...
else 
{
    ...
    $result = $Realm->insert('account', array('username' => $_POST['account'], 'sha_pass_hash' => $password));
    if($result !== FALSE)
    {
        // Get the account ID
