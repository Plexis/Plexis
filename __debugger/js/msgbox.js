                ...
                this.esqueleto.msgbox.queue(this.options.name, $.proxy(function( next ) {
                    ...
                    if (typeof inputs !== "undefined")
                    {
                        ...
                        $.each(inputs, $.proxy(function(i, input) {
                            ...
                            {
                                iLabel = input.label ? '<label class="'+this.options.name+'-label">'+input.label +'</label><br />': '';
                                input.value = input.value === undefined ? '' : input.value;
                                iName  = input.name === undefined ? this.options.name+'-label-'+i : input.name;
                                this.esqueleto.inputs.append($(iLabel+'<textarea name="'+iName+'">'+input.value+'</textarea>'));
                            }
                            else if(input.type == 'select')
                            {
                                ...
                                pre = iLabel +'<select name="'+iName+'">';
                                jQuery.each(input.options, function(k, v) {
                                    pre += '<option value="' + k + '">' + v +'</option>';
                                });
                                pre += '</select>';
                                
                                this.esqueleto.inputs.append($( pre ));
                            }
                            else
                            {
                                ...
                                iName  = input.name === undefined ? this.options.name+'-label-'+i : input.name;
                                this.esqueleto.inputs.append($(iLabel+'<input type="'+input.type+'" name="'+iName+'" value="'+input.value+'" '+iRequired+'/>'+fLabel));
                            }
                        }, this));
                    }
                        ...
                        $.each(buttons, $.proxy(function(i, button) {
                            if (button.type == 'submit')
                            {
                                this.esqueleto.buttons.append($('<button type="submit">'+button.value+'</button>').bind('click', $.proxy(function(e) {
                                    ...
                                    {
                                        array = new Array();
                                        ar1 = this.toArguments($('input', this.esqueleto.inputs));
                                        if(ar1 != null) array = $.merge(array, ar1);
                                        ar2 = this.toArguments($('textarea', this.esqueleto.inputs));
                                        if(ar2 != null) array = $.merge(array, ar2);
                                        ar3 = this.toArguments($('select', this.esqueleto.inputs));
                                        if(ar3 != null) array = $.merge(array, ar3);
                                        this.close(array);
                                    }
